...
const lineChart = async (req, res) => {
  ...
  let lead = await Lead.find(query)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
  ...
  let account = await Account.find(senderQuery)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
  ...
  let emailTemp = await EmailTemp.find(senderQuery)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
  ...
  let opprtunities = await Opprtunities.find(senderQuery)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
    .exec();
  const OpprtunitiesData = opprtunities.filter(
    (item) => item?.createBy !== null
  );

  let invoices = await Invoices.find(senderQuery)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
  ...
  let quotes = await Quotes.find(senderQuery)
    .populate({
      path: "createBy",
      match: { deleted: false },
    })
  ...
  const mergedRoles = userDetails?.roles?.reduce((acc, current) => {
    current?.access?.forEach((permission) => {
      const existingPermissionIndex = acc.findIndex(
  ...
  let result = [
    ...
    {
      name: "Opportunities",
      length: OpprtunitiesData?.length,
      color: "linkedin",
    },
  ...
  let activeModulesReport = result?.filter((item) =>
    activeModules?.includes(item?.name)
  );
